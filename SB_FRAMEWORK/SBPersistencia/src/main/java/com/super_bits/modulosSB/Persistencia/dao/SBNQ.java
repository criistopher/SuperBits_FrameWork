...
public class SBNQ implements Serializable {
    ...
    private void aplicaValoresParametro() {
        ...
        int qtdParametrosEnvidos = getQuantidadeParametros();
        Query queryTemp = getQueryHibernate();
        int qtdParametrosEncontrados = queryTemp.getParameters().size();

        for (Parameter<?> pr : queryTemp.getParameters()) {

            Class<?> tipoParametro = pr.getParameterType();
            String nomeParametro = pr.getName();
            Integer posicao = pr.getPosition();

            if (temParamInfo()) {
                queryTemp.getParameter(pr.getName());
                try {

                    queryTemp.setParameter(pr.getName(),
                            parametros.get(pr.getName()));

                } catch (Exception e) {
                    FabErro.SOLICITAR_REPARO.paraDesenvolvedor("erro adcionando parametro inc " + "erro adcionando parametro inc ", e);

                }

            } else {
                try {
                    queryTemp.setParameter(pr.getPosition(),
                            valoresParametros[pr.getPosition() - 1]);

                } catch (Exception e) {

                    FabErro.SOLICITAR_REPARO.paraDesenvolvedor("erro adcionando parametro  simples", e);
                    //CentralDeMensagens.erroSQL(
                    //		"erro adcionando parametro  simples"
                    //				+ e.getMessage(), this,e);
                }
    ...
    private void makeQueryHibernate() {
        if (queryHibernate == null) {
            try {
                ...
                FabErro.SOLICITAR_REPARO.paraDesenvolvedor("\"erro Criando HSQL em SBNQ.makeQueryHibernate", e);
    ...
    public String getSql() {
        if (temParamInfo()) {
            ...
            return UtilSBCoreStrings.substituiParametrosNomeadosPorInterroga(sql);
        }
    }
