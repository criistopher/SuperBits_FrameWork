...
import static com.super_bits.Controller.ControllerAppAbstratoSBCore.getPermissoes;
...
public class UtilSBController {
    ...
    public static ItfAcaoDoSistema obterAcaoByMethodo(Method pMetodo) {
        ...
        for (Annotation an : anotacoes) {
            ...
            Method[] metodos = an.annotationType().getDeclaredMethods();
            for (Method fld : metodos) {
                if (fld.getName().equals("acao")) {

                    try {
                        ItfFabricaModulos fabrica = (ItfFabricaModulos) fld.invoke(an);
                        ...
                        FabErro.PARA_TUDO.paraSistema("A acao do metodo" + pMetodo.getName() + " não contem uma anotação extendendo Itffabrica, contendo o campo acao, desta forma não é possível determinar o nível de permissão deste método", t);
            ...
            for (Annotation a : anotacoes) {
                ...
                try {
                    ...
                    try {
                        ...
                        ItfAcaoDoSistema acaosis = (ItfAcaoDoSistema) acao.getRegistro();
                        acaosis.setId(UtilSBController.gerarIDMetodoAcaoDoSistema(pMetodo));
                        acaosis.setIdMetodo(UtilSBController.gerarIDMetodoAcaoDoSistema(pMetodo));
                        return acaosis;
