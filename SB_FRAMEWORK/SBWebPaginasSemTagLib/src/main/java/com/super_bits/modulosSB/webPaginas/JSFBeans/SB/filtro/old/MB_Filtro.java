...
public abstract class MB_Filtro<T extends ItfBeanSimples> extends ClasseTipada implements Serializable {
    ...
    public String getNomeFiltroUrlAmigavel() {
        return UtilSBCoreStrings.makeStrUrlAmigavel(nomeFiltro);
    }
    ...
    public void setOpcaoAtualByNome(String pNomeOpcao) {
        ...
        for (OpcaoFiltro op : opcoes) {
            if (op.getNomeCurto().equalsIgnoreCase(pNomeOpcao) || op.getNomeCurtoURLAmigavel().equalsIgnoreCase(UtilSBCoreStrings.makeStrUrlAmigavel(pNomeOpcao))) {
                setOpcaoAtual(op);
                return;
            }
        }

        UtilSBWP_JSFTools.mensagens().erroSistema("Item" + pNomeOpcao + " ou " + UtilSBCoreStrings.makeStrUrlAmigavel(pNomeOpcao) + "não encontrado no filtro" + this.getClass().getSimpleName());
    }
