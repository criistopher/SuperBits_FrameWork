        ...
        if (pEfeito.getParametros().size() > 0) {
            for (EfeitoImagem.PARAMETRO_EFEITO pr : pEfeito.getParametros().keySet()) {
                if (pEfeito.getParametros().get(pr).equals("0")) {
                    FabMensagens.enviarMensagemUsuario("Parametro obrigatório: '" + pr + "' do efeito " + pEfeito.getTipoEfeito().toString() + " não foi configurado", FabMensagens.AVISO);
                } else {
                    efeitos.add(pEfeito);
                }
            }
    ...
    public void processarImagem() {
        for (EfeitoImagem efeito : efeitos) {
            try {
                ...
                FabMensagens.enviarMensagemUsuario("Erro aplicando efeito" + efeito.getNomeCurto(), FabMensagens.ERRO);
            }
